...
def build(rows_from=None) -> MCPServer:
    ...
    @server.tool(
        ...
        description=(
            ...
            "wire_read for the text. Two sources title an item with a bare name — "
            "producthunt with the product's, hub with the repo id — so for those a "
            "listing that looks like nothing happened is the body being where the "
            "meaning is. detail='full' includes each stored body inline and "
    ...
    @server.tool(
        ...
        description=(
            ...
            "must never be reported as such — read the DOWN, CEILING and UNKNOWN "
